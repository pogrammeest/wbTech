...
class AccountViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = Account.objects.create_user(self.username, self.password)
        self.user.save()
    ...
    def test_account_list_un_authenticated(self):
        self.api_authentication(user=None)
        response = self.client.get(self.list_url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_user_detial(self):
        temp_user = Account.objects.get(username='user0')

        response = self.client.get(reverse("api:account-detail", kwargs={'pk': temp_user.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_post_create_get_ordering(self):
        user_pool = Account.objects.all()[:3]
        data = [{
            'title': user.username,
            'content': f'content by {user.username}',
        } for user in user_pool]
        for i in range(len(user_pool)):
            self.api_authentication(user_pool[i])
        ...
        self.api_authentication(user_pool[1])
        self.client.post(reverse("api:post-list"), data[1])

        response = self.client.get(f'{reverse("api:account-list")}?ordering=posts')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_sub_un_sub_and_feed(self):
        ...
        for user in Account.objects.all()[4:7]:
    ...
    def test_post_liked_unlike(self):
        user_pool = Account.objects.all()[:3]
        data = [{
            'title': user.username,
            'content': f'content by {user.username}',
        ...
        for i in range(len(user_pool)):
            ...
            response = self.client.post(reverse("api:post-list"), data[i])
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
            last_user = response.data['author']['username']
        author = Account.objects.get(username=last_user)
        post = Post.objects.get(author=author)
        for i in range(len(user_pool)):
            self.api_authentication(user_pool[i])
            self.client.get(reverse("api:post-like", kwargs={
                'pk': post.pk
            }))
